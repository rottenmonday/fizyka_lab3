...
Gdzie $q_l$ jest ciepłem topnienia lodu, $m_l$ to masa wrzucanego lodu, $c_{wH2O}$ to ciepło właściwe wody (bierzemy ciepło właściwe tablicowe, ponieważ to wyliczone w zadaniu 1 ma zbyt odbiegające wartości), $c_{wk}$ to ciepło właściwe kalorymetru wyliczone w 2 zadaniu, $T_k$ to temperatura końcowa układu, $m_{1H2O}$ to masa wody w kalorymetrze przed dodaniem lodu, $T_{1H2O}$ to temperatura układu przed dodaniem lodu, $T_k$ to temperatura końcowa układu, $m_k$ to masa kalorymetru.
...
Poszczególne niepewności pomiarowe przyjęliśmy jako: niepewność pomiarowa $u_B(c_{wH_2O}) = 0$ (bierzemy wartość tablicową), niepewność $u_B(m_{1H2O}) = \frac{0.05}{\sqrt{3}}$, $u_B(c_{wk})=517.22$, $u_B(m_l)=\frac{0.0001}{\sqrt{3}}$, $u_B(T_k)=\frac{0.0001}{\sqrt{3}}$, $u(T_{1H_2O})=\frac{0.0001}{\sqrt{3}}$.
...
p <- ggplot(as.data.frame(pomiary_mod), aes(x=V3, y=wyniki)) + geom_point() +
   ...
   ggtitle("Zależność wyliczonego ciepła topnienia lodu od masy Seria 1")
p
p <- ggplot(wyniki_wozniak, aes(x=V3, y=wyniki)) + geom_point() +
   geom_errorbar(aes(ymin=wyniki-bledy, ymax=wyniki+bledy)) +
   labs(x = "Masa początkowa wody[kg]", y = "Ciepło topnienia lodu[J/kg]") +
   ggtitle("Zależność wyliczonego ciepła topnienia lodu od masy Seria 2")
p
p <- ggplot(wyniki_lopatowski, aes(x=V3, y=wyniki)) + geom_point() +
   geom_errorbar(aes(ymin=wyniki-bledy, ymax=wyniki+bledy)) +
   labs(x = "Masa początkowa wody[kg]", y = "Ciepło topnienia lodu[J/kg]") +
   ggtitle("Zależność wyliczonego ciepła topnienia lodu od masy Seria 3")
p
