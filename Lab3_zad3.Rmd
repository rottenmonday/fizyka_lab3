...
Choć krok ten jest nieobowiązkowy celem eksperymentu jest wyznaczenie wartości ciepła właściwego wody w warunkach domowyc, by utrwalić w praktyce wiedzę teoretyczną i "pobawić" się w naukowca.

## Opis eksperymentu

Ciepło właściwe jest stosunkiem potrzebnej energi do zmmieny temperatury o jeden stopień ciała o masie 1kg. 
By wyznaczyć tę wartość dla wody potrzebujemy zatem 3 informaci. 
Masy wody, różnicy temperatur i ilości dostarczonej energii zmienionej na energię kinetyczną cząstek, w tym przypadku wody. 
$$
c_w = \frac{Q}{m * \Delta T}
$$

Ponieważ eksperyment będzie przeprowadzany w warunkach domowych skorzystamy z przyrządów dostępnych i zastosujemy pewne uproszczenia. 
Do wyznaczenia energi dostarczonej wykorzystamy czajnik elektryczny o pewnej mocy grzałki i stoper. 
$$
Q = P * t
$$
Założymy, że cała energia pochodząca z grzałki podgrzewałą wodę (100% wydajności grzałki, co jest bardzo mało prawdopodobne). 
W roli stoperu użyty zostanie smartfon.
Do pomiaru masy użyta zostanie waga kuchenna sprężynowa. 
Zakładamy przy tym, że przy przelewaniu wody do czajnika cała woda zostanie przelana, oraz że w czajniku nie znajduje się przed dolaniem ani kropla wody. 
W trakcie podgrzewania część wody wyparowuje jednak świadomie zaniedbujemy te utratę masy.

Ostateczny wzór użyty do wyznaczenia wartośći ciepła właściwego
$$
c_{H_2O} = \frac{P * \Delta t}{m *  \Delta T}
$$
Gdzie $P$ to moc grzałki czajnika, $m$ to masa podgrzewanej wody, $\Delta t$ to ilość czasu potrzebna na podgrzanie wody do temperatury ok. 100 stopni Celcjusza, $\Delta T$ to różnica temperatur po podgrzaniu i przed podgrzaniem.

## Układ eksperymentalny

Pomiary wykonywane były przy użyciu termometru bimetalowego z zakresem -40+1000°C, dokładnością ±0.75%±3cyfry, rozdzielczość 1°C. 
Waga kuchenna analogowa sprężynowa z dokładnością 10 g. 
Czajnik użyty do gotowania wody został uprzednio wyczyszczony 10% roztworem octu, ma moc znamionową 1850-2200W. 
Do mierzenia czasu użyto dwóch różnych smartfonów. 
Sposób rozpoczęcia odliczania był "3,2,1,już" co wydawało się zwiększać znacząco średni czas reakcji człowieka; niestety brak danych o ile.
Szybki eksperyment z włączeniem i wyłączeniem stopera wykazał czas reakcji człowieka na około 160ms.
Odliczanie było kończone jeżeli miernik pokazał liczbę >=100, i odczyt temperatury następował zaraz po tym. 
Po odczytaniu temperatury gorącą wodę ponownie mierzono używając tej samej wagi. 
Ważenie składało się z dwóch pomiarów, czasem trzeba było wstrząsnąć wagą. 
Woda używana pochodziła z instalacji domowej (z kranu); prawdopodobnie była ona bardzo twarda.
...
Niepewności poszczególnych pomiarów są typu B. 
Niepewność wyznaczonej wartości liczymy z prawa propagacji niepewności.
...
Wartośc tablicowa ciepła właściwego wody wynosi około $4190 \frac{J}{kg * K}$. 
Wartość ta nie znajduje się w przedziałach niepewności wyników, co śwaidczy o grubym błędzie w pomiarach parametrów. 
Wynikać to może z założeń dotyczących wydajności grzałki;
inną możliwością może być stara waga.
...
Do kalorymetru o znanej masie i cieple właściwym wlewamy pewną początkową ilość wody $m_{1H_2O}$. Czekamy na ustalenie się równowagi termodynamicznej i mierzymy temperaturę układu. Następnie do układu dodajemy kostkę lodu o znanej masie i ponownie czekamy na ustalenie się równowagi termodynamicznej. 

Bilans energetyczny takiego układu wygląda tak:

$$ q_lm_l+m_lc_{w H_2O}T_k = m_{1H_2O}c_{wH_2O}(T_{1H_2O}-T_k) + m_kc_{wk}(T_{1H_2O}-T_k)$$
Gdzie $q_l$ jest ciepłem topnienia lodu, $m_l$ to masa wrzucanego lodu, $c_{wH_2O}$ to ciepło właściwe wody (bierzemy ciepło właściwe tablicowe, ponieważ to wyliczone w zadaniu 1 ma zbyt odbiegające wartości), $c_{wk}$ to ciepło właściwe kalorymetru wyliczone w 2 zadaniu, $T_k$ to temperatura końcowa układu, $m_{1H_2O}$ to masa wody w kalorymetrze przed dodaniem lodu, $T_{1H_2O}$ to temperatura układu przed dodaniem lodu, $T_k$ to temperatura końcowa układu, $m_k$ to masa kalorymetru.
...
kable(pomiary_mod, caption="Pomiary i wartości ciepła topnienia Seria 1", format = "latex", escape = FALSE, align = 'c', col.names = c("$T_{1H_2O}[C]$","$T_k[C]$","$m_{1H_2O}[kg]$","$c_t[J/kg]$")) %>%
   kable_styling(position="center", latex_options = c("striped", "HOLD_position"))

kable(pomiary_mod_wozniak, caption="Pomiary i wartości ciepła topnienia Seria 2", format = "latex", escape = FALSE, align = 'c', col.names = c("$T_{1H_2O}[C]$","$T_k[C]$","$m_{1H_2O}[kg]$","$c_t[J/kg]$")) %>%
   kable_styling(position="center", latex_options = c("striped", "HOLD_position"))

kable(pomiary_mod_lopatowski, caption="Pomiary i wartości ciepła topnienia Seria 3", format = "latex", escape = FALSE, align = 'c', col.names = c("$T_{1H_2O}[C]$","$T_k[C]$","$m_{1H_2O}[kg]$","$c_t[J/kg]$")) %>%
   kable_styling(position="center", latex_options = c("striped", "HOLD_position"))
...
Za poszczególne niepewności pomiarowe przyjęliśmy: $u_B(m_{1H_2O}) = \frac{0.05}{\sqrt{3}}$, $u_B(c_{wk})=517.22$, $u_B(m_l)=\frac{0.0001}{\sqrt{3}}$, $u_B(T_k)=\frac{0.0001}{\sqrt{3}}$, $u(T_{1H_2O})=\frac{0.0001}{\sqrt{3}}$. Podobnie jak w poprzednim eksperymencie, wartość ciepła właściwego wody uznano za nieobarczoną niepewnością pomiarową.
...
kable(pomiary_mod, caption="Wartości ciepła topnienia i niepewności Seria 1", format = "latex", escape = FALSE, align = 'c', col.names = c("$T_{1H_2O}[C]$","$T_k[C]$","$m_{1H_2O}[kg]$","$c_t[J/kg]$", "$u_B(c_t)[J/kg]$")) %>%
   kable_styling(position="center", latex_options = c("striped", "HOLD_position"))

kable(pomiary_mod_wozniak, caption="Wartości ciepła topnienia i niepewności Seria 2", format = "latex", escape = FALSE, align = 'c', col.names = c("$T_{1H_2O}[C]$","$T_k[C]$","$m_{1H_2O}[kg]$","$c_t[J/kg]$", "$u_B(c_t)[J/kg]$")) %>%
   kable_styling(position="center", latex_options = c("striped", "HOLD_position"))
kable(pomiary_mod_lopatowski, caption="Wartości ciepła topnienia i niepewności Seria 3", format = "latex", escape = FALSE, align = 'c', col.names = c("$T_{1H_2O}[C]$","$T_k[C]$","$m_{1H_2O}[kg]$","$c_t[J/kg]$", "$u_B(c_t)[J/kg]$")) %>%
   kable_styling(position="center", latex_options = c("striped", "HOLD_position"))
```

## Wykresy

Prześledźmy jeszcze zależność wartości wyliczonego ciepła topnienia lodu od masy początkowej wody w kalorymetrze, wraz zaznaczonymi niepewnościami:
...
Jako wynik naszych obliczeń weźmiemy średnią arytmetyczną wartości ciepła topnienia lodu, a jako niepewność pomiarową - największą z uzyskanych niepewności. W takim razie wyliczona przez nas wartość ciepła topnienia lodu wynosi: `r round(srednia, 2)`(`r round(err, 2)`)[J/kg].
...
Wartości wyliczonego ciepła topnienia lodu w zasadzie nie zależą od masy początkowej wody. Jedyne co udaje nam się przez zwiększenie masy początkowej wody to zwiększenie precyzji obliczeń - niepewności maleją.
