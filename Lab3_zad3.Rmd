...
kable(pomiary_mod, caption="Pomiary i wartości ciepła topnienia", format = "latex", escape = FALSE, align = 'c', col.names = c("$T_{1H_2O}[C]$","$T_k[C]$","$m_{1H_2O}[kg]$","$c_t[J/kg*K]$")) %>%
   kable_styling(position="center", latex_options = c("striped", "HOLD_position"))
...
kable(pomiary_mod, caption="Wartości ciepła topnienia i niepewności", format = "latex", escape = FALSE, align = 'c', col.names = c("$T_{1H_2O}[C]$","$T_k[C]$","$m_{1H_2O}[kg]$","$c_t[J/kg*K]$", "$u_B(c_t)[J/kg*K]$")) %>%
   kable_styling(position="center", latex_options = c("striped", "HOLD_position"))
```
...
p <- ggplot(wyniki, aes(x=V3, y=wyniki)) + geom_point() + geom_line() +
#   geom_errorbar(aes(ymin=wyniki-bledy, ymax=wyniki+bledy)) +
   labs(x = "Masa początkowa wody[kg]", y = "Ciepło topnienia lodu[J/kg*K]") +
   ggtitle("Zależność wyliczonego ciepła topnienia lodu od masy początkowej wody")
p

#plot(x = pomiary_mod[,3], y = pomiary_mod[,4], type="p", xlab = "Masa początkowa wody", ylab="Ciepło topnienia lodu")
#lines(x = pomiary_mod[,3], y = pomiary_mod[,4])
```
